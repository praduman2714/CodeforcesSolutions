...
public class MainClass {
    ...
    private static void solve() {
        int n = fs.nextInt();
        String num = fs.next();
        char ld = num.charAt(0);
        String t = "1";
        String st = "1" +t.repeat(n-1) + "1";
        if(ld == '9' ){
            //StringBuilder res = new StringBuilder();

            BigInteger from = new BigInteger(st);
            BigInteger to = new BigInteger(num);
            BigInteger res = from.subtract(to);

            fw.out.println(res.toString());
        }else{
            for(int i = 0; i<n; i++){
                fw.out.print('9' - num.charAt(i));
            }
            fw.out.println();
        }
    }
