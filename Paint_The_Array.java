...
public class MainClass {
    ...
    private static void solve() {
        ...
        //long gcdo = -1, gcde =-1;
        long gcdo = (al.size() == 1) ? al.get(0) : gcd(al.get(0), al.get(1));
        long gcde = (al1.size() == 1) ? al1.get(0) : gcd(al1.get(0), al1.get(1));

        for(int i = 2; i<al.size(); i++){
            gcdo = gcd(gcdo, al.get(i));
        }
        for(int i = 2; i<al1.size() ; i++){
            gcde = gcd(gcde, al1.get(i));
        }
        ...
        //fw.out.println((gcde == gcdo) ? 0 : Math.max(gcde, gcdo));
